...
### 다형성 활용 1
- 타입이 다르면 **중복 코드 제거가 어려운 문제**가 발생


## 추상 클래스

### 추상 클래스란?
- **부모 클래스는 제공되지만, 실제 생성되면 안 되는 클래스**
- **추상적인 개념을 제공하며, 인스턴스를 생성할 수 없음**
...
### 추상 메서드
- **자식 클래스가 반드시 오버라이딩해야 하는 메서드를 부모 클래스에 정의**
- **메서드 바디({})가 없음**
...
- **추상 메서드는 자식 클래스에서 반드시 오버라이딩해야 한다.**
...
### 인터페이스란?
- **순수 추상 클래스를 더 편리하게 사용할 수 있도록 제공하는 기능**
- **모든 메서드가 `public abstract` (생략 가능)**
- **다중 구현(다중 상속) 지원**
...
| 개념 | 설명 |
|------|------|
| **상속(Inheritance)** | 부모의 기능을 물려받는 것이 목적 |
| **구현(Implementation)** | 인터페이스의 모든 메서드를 자식 클래스가 직접 구현해야 함 |

### 인터페이스를 사용해야 하는 이유
1. **제약**: 특정 클래스가 반드시 특정 메서드를 구현하도록 강제
2. **다중 구현**: 자바는 다중 상속을 지원하지 않지만, 인터페이스를 통해 다중 구현 가능
